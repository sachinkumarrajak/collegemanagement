...
namespace collegemanegement
{
    public partial class facultyupdate : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                cmd = new OdbcCommand("select * from faculty where id='" + TextBox1.Text + "'", cn);
                cn.Open();
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                cmd = new OdbcCommand("update faculty  set name=?,gender=?,dob=?,email=?,phone=?,father_nm=?,mother_nm=?,addres=?,states=?,city=?,dept=?,post=?,subject=?  where id=?", cn);
